...
const self = module.exports = {
  createPoolConnection: async () => mysql.createPool({
    connectionLimit: 20,
  ...
  createTable: async (dbName, tableName, fields) => {
    const query = `SELECT * FROM information_schema.tables WHERE table_schema = '${dbName}' AND table_name = '${tableName}' LIMIT 1;`
    const tables = await self.safeQuery(query)
    if (tables.length > 0) return `TABLE ${tableName} ALREADY EXISTS, SKIPPING`
    else {
      await self.safeQuery(`create table ${tableName} ${fields}`)
  ...
  updateTableColumn: async (dbName, tableName, column) => {
    ...
    const query = `SELECT IF(count(*) = 1, 'Exist','Not Exist') AS result FROM information_schema.columns WHERE table_schema = '${dbName}' AND table_name = '${tableName}' AND column_name = '${column.name}';`
    ...
    if (columnExists[0].result === 'Exist') return `COLUMN: ${column.name} ALREADY EXISTS IN TABLE: ${tableName}, SKIPPING`
      ...
      const query = `ALTER TABLE ${tableName} ADD COLUMN ${column.name} ${column.sqlType} ${column.sqlDef}`
