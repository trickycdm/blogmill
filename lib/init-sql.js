...
const schema = require(`${_root}/cms/schema`)
let tables = []

for (let page of schema.pages) {
  let table = {db: process.env.DB_NAME, name: page.table}
  let fields = []
  ...
  updateColumns: async () => {
    try {
      for (let table of tables) {
        for (let column of table.rawFields) console.log(await db.updateTableColumn(table.db, table.name, column))
      }
