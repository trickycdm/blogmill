...
router.get('/media/:id', async (req, res, next) => {
  try {
    if (!req.params.id) return res.sendFile(`${ROOT}/${CMS_CONFIG.CMS_ROOT}/${CMS_CONFIG.DEFAULT_IMG_PATH}`)
    ...
    if (!mediaObj) res.sendFile(`${ROOT}/${CMS_CONFIG.CMS_ROOT}/${CMS_CONFIG.DEFAULT_IMG_PATH}`)
    else res.sendFile(`${ROOT}/${CMS_CONFIG.RELATIVE_IMG_UPLOADS_DIR}/${mediaObj.file_name}`)
  } catch (err) {
    console.error(err)
    res.sendStatus(500)
  }
...
router.get('/media', async (req, res, next) => {
  return res.sendFile(`${ROOT}/${CMS_CONFIG.CMS_ROOT}/${CMS_CONFIG.DEFAULT_IMG_PATH}`)
})
